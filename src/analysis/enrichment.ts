...
import { SymbolNode, EdgeFact } from './types';
...
export interface ComplexityMetrics {
  ...
  classes: number;
  maxNesting: number;
  cyclomaticProxy: number;
  isHotspot: boolean;
}

export interface CodeSmell {
  file: string;
  kind: 'god-file' | 'long-method' | 'high-complexity' | 'deep-nesting' | 'too-many-params' | 'large-class';
...
export function analyzeComplexity(relPath: string, content: string): ComplexityMetrics {
  ...
  for (const line of lines) {
    ...
    if (/\b(function|def|fn|func|sub)\b/.test(line)) { functions++; }
    if (/\b(class|struct|impl)\b/.test(line)) { classes++; }
  }
...
export function detectSmells(relPath: string, content: string, nodes: SymbolNode[]): CodeSmell[] {
  ...
  for (const line of lines) {
    for (const ch of line) {
      if (ch === '{') { nest++; maxNest = Math.max(maxNest, nest); }
...
export function mapTestFiles(allFiles: string[]): EdgeFact[] {
...
function guessSourceForTest(testFile: string): string | undefined {
  ...
  // Try common patterns
  if (src !== testFile) { return src; }
  return undefined;
}

// ── API Surface Mapping ─────────────────────────────────────────────────────

export function extractApiSurface(relPath: string, content: string, namespace?: string): { nodes: SymbolNode[]; edges: EdgeFact[] } {
