...
import { SymbolNode, EdgeFact, NodeKind } from './types';
...
function symbolId(relPath: string, kind: NodeKind, name: string): string {
...
export function parseTsJs(relPath: string, content: string, namespace?: string): ParseResult {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const ch of line) {
      ...
      if (ch === '}') {
        braceDepth--;
        if (braceDepth <= classDepth) { currentClass = undefined; classDepth = -1; }
        if (braceDepth <= fnDepth) { currentFunction = undefined; fnDepth = -1; }
      }
    }
    ...
    if (fnMatch) {
      ...
      const kind: NodeKind = fnName.startsWith('test') || fnName.startsWith('it') || fnName.startsWith('describe') ? 'test' : 'function';
    ...
    if (currentClass) {
      ...
      if (methodMatch && !['if', 'for', 'while', 'switch', 'catch', 'return', 'new', 'typeof'].includes(methodMatch[1])) {
        ...
        const kind: NodeKind = methodName.startsWith('test') ? 'test' : 'method';
...
export function parsePython(relPath: string, content: string, namespace?: string): ParseResult {
  const nodes: SymbolNode[] = [];
  const edges: EdgeFact[] = [];
  const errors: string[] = [];
  const lines = content.split('\n');
  ...
  let currentClass: string | undefined;
  let currentFunction: string | undefined;
  const indentStack: { name: string; indent: number; kind: 'class' | 'function' }[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while (indentStack.length > 0 && indent <= indentStack[indentStack.length - 1].indent) {
      ...
      if (popped.kind === 'class') { currentClass = undefined; }
      if (popped.kind === 'function') { currentFunction = undefined; }
    }
    ...
    if (fnMatch) {
      ...
      const kind: NodeKind = isTest ? 'test' : isMethod ? 'method' : 'function';
...
export function parseRust(relPath: string, content: string, namespace?: string): ParseResult {
  const nodes: SymbolNode[] = [];
  const edges: EdgeFact[] = [];
  const errors: string[] = [];
  const lines = content.split('\n');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (fnMatch) {
      ...
      const kind: NodeKind = isTest ? 'test' : currentImpl ? 'method' : 'function';
      const sid = symbolId(relPath, kind, label);
      nodes.push({ id: sid, kind, label, file: relPath, line: n, namespace });
...
export function parseGeneric(relPath: string, content: string, namespace?: string): ParseResult {
  const nodes: SymbolNode[] = [];
  const edges: EdgeFact[] = [];
  const fid = fileId(relPath);
  nodes.push({ id: fid, kind: 'file', label: relPath, file: relPath, line: 1, namespace });
...
export function parseFile(relPath: string, content: string, namespace?: string): ParseResult {
