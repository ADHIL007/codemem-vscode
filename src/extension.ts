...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  // ── Install Server Help ───────────────────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand('codemem.installServerHelp', async () => {
      const choice = await vscode.window.showInformationMessage(
        'CodeMem Server is required. You can install codemem-server-v2 from GitHub and run it locally.',
        { modal: true, detail: 'Repository: https://github.com/ADHIL007/codemem-server-v2\n\nQuick setup:\n  1. git clone https://github.com/ADHIL007/codemem-server-v2.git\n  2. cd codemem-server-v2\n  3. Follow the README instructions to start the server\n  4. Set codemem.serverUrl in VS Code settings to your server URL' },
        'Open GitHub Repo',
        'Clone & Setup in Terminal',
      );
