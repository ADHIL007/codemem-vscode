  ...
  // save and pushes updated edges to codemem without a full workspace scan.
  {
    ...
    context.subscriptions.push(
      vscode.workspace.onDidSaveTextDocument(async (doc) => {
        ...
        if (!cfg().get<boolean>('copilotHookIndexing', true)) { return; }
        ...
        if (!doc.uri.fsPath.startsWith(wsRoot)) { return; }
        ...
        if (IGNORED_DIRS.some((d) => relPath.startsWith(d + '/'))) { return; }

        // Check server is reachable
        const serverUrl = cfg().get<string>('serverUrl', 'http://localhost:4242');
        const namespace = resolveNamespace(cfg().get<string>('namespace', '') ?? '') || wsFolders[0].name;

        try {
          const { parseFile: _parseFile, UploadQueue: _UploadQueue } = await import('../analysis');
          ...
          const queue = new _UploadQueue({
            chunkSize: cfg().get<number>('chunkSize', 60),
            maxRetries: 2,
            retryDelayMs: 500,
            namespace,
            serverUrl,
