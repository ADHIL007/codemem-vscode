...
const watcher_1 = require("../analysis/watcher");
const MEMORY_TYPES = ['decision', 'pattern', 'preference', 'style', 'habit', 'insight', 'context'];
function registerCommands(context, client, memoriesProvider, sessionsProvider, statusBar) {
    const cfg = () => vscode.workspace.getConfiguration('codemem');
    const ns = () => (0, memoriesProvider_1.resolveNamespace)(cfg().get('namespace', '') ?? '');
    // -- connect ------------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.connect', async () => {
        const url = cfg().get('serverUrl', 'http://localhost:4242');
        ...
        if (ok) {
            ...
            vscode.window.showErrorMessage(`CodeMem: cannot reach server at ${url}. Start it with: codemem serve --api`);
        }
    }));
    // -- refresh ------------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.refresh', () => {
        memoriesProvider.refresh();
        sessionsProvider.refresh();
    }));
    // -- search -------------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.search', async () => {
        ...
        const results = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Searching CodeMem...' }, () => client.search({ query, namespace: ns(), k: 20 }));
        if (results.length === 0) {
        ...
        if (pick) {
            await showMemoryDetail(pick.result);
        }
    }));
    // -- storeSelection -----------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.storeSelection', async () => {
    ...
    // -- storeClipboard -----------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.storeClipboard', async () => {
    ...
    // -- showStats ----------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.showStats', async () => {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('codemem.stats', 'CodeMem Stats', vscode.ViewColumn.Beside, {});
    ...
    // -- openUI -------------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.openUI', () => {
        const url = cfg().get('serverUrl', 'http://localhost:4242');
        vscode.env.openExternal(vscode.Uri.parse(url));
    }));
    // -- configureTeamMCP ---------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.configureTeamMCP', async () => {
        ...
        let serverUrl = cfg().get('serverUrl', 'http://localhost:4242');
        if (mode.value === 'http') {
        ...
        try {
            await (0, mcpConfig_1.writeMcpConfig)(workspaceRoot, mode.value, serverUrl);
            vscode.window.showInformationMessage(`CodeMem: .mcp.json updated in ${workspaceRoot}. Reload Claude Code to apply.`);
        }
        catch (err) {
    ...
    // -- deleteMemory -------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.deleteMemory', async (node) => {
        const confirm = await vscode.window.showWarningMessage(`Delete memory: "${truncate(node.memory.content, 50)}"?`, { modal: true }, 'Delete');
        if (confirm !== 'Delete') {
            return;
        }
        try {
    ...
    // -- copyMemoryContent --------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.copyMemoryContent', async (node) => {
    ...
    // -- copyMemoryId -------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.copyMemoryId', async (node) => {
        await vscode.env.clipboard.writeText(node.memory.id);
        vscode.window.showInformationMessage(`CodeMem: ID ${node.memory.id} copied`);
    }));
    // -- registerRepo -------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.registerRepo', async () => {
        ...
        try {
            ...
            const doInit = await vscode.window.showInformationMessage('Initialize workspace with CodeMem hooks, agents & MCP config?', 'Yes', 'No');
            if (doInit === 'Yes') {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codemem.analyzeRepo', async () => {
        // All analysis is local-first  -  redirect to analyzeWorkspace
        await vscode.commands.executeCommand('codemem.analyzeWorkspace');
    }));
    // -- listRepos ----------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.listRepos', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codemem.localAnalyzeAndSync', async () => {
        await vscode.commands.executeCommand('codemem.analyzeWorkspace');
    }));
    // -- doctor -------------------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.doctor', async () => {
        const serverUrl = cfg().get('serverUrl', 'http://localhost:4242');
        const embedUrl = cfg().get('embeddingUrl', 'https://integrate.api.nvidia.com/v1');
        const embedModel = cfg().get('embeddingModel', 'nvidia/nv-embed-v1');
        const embedKey = cfg().get('embeddingApiKey', '');
        memoriesProvider.clearDoctorResults();
        const checks = [];
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'CodeMem Doctor: running checks...', cancellable: false }, async (progress) => {
            // 1. Server health
            progress.report({ message: 'Checking server...' });
            try {
                const ok = await client.health();
                checks.push(new memoriesProvider_1.DoctorCheckNode('Server reachable', ok, ok ? serverUrl : `Cannot reach ${serverUrl}`));
            }
            catch (err) {
                checks.push(new memoriesProvider_1.DoctorCheckNode('Server reachable', false, String(err)));
            }
            // 2. DB / stats
            progress.report({ message: 'Checking database...' });
            try {
                const stats = await client.getStats();
                checks.push(new memoriesProvider_1.DoctorCheckNode('Database accessible', true, `${stats.memory_count} memories * ${stats.node_count} nodes * ${stats.edge_count} edges`));
            }
            catch (err) {
                checks.push(new memoriesProvider_1.DoctorCheckNode('Database accessible', false, String(err)));
            }
            // 3. MCP endpoint
            ...
            try {
                ...
                checks.push(new memoriesProvider_1.DoctorCheckNode('MCP endpoint', res.ok || res.status === 200, `${mcpUrl}  -  HTTP ${res.status}`));
            }
            catch (err) {
                checks.push(new memoriesProvider_1.DoctorCheckNode('MCP endpoint', false, `${mcpUrl}  -  ${String(err)}`));
            }
            // 4. Embedding API
            progress.report({ message: 'Checking embedding API...' });
            const provider = cfg().get('embeddingProvider', 'nvidia-nim');
            const isLocal = provider === 'ollama';
            if (!embedKey && !isLocal) {
                checks.push(new memoriesProvider_1.DoctorCheckNode('Embedding API key', false, 'Not configured  -  run Analyze Workspace to set up a provider'));
            }
            else {
                try {
                    const docHeaders = { 'Content-Type': 'application/json' };
                    if (embedKey) {
                        docHeaders['Authorization'] = `Bearer ${embedKey}`;
                    }
                    ...
                    checks.push(new memoriesProvider_1.DoctorCheckNode(`Embedding API (${provider})`, ok, ok
                        ? `${embedModel} @ ${embedUrl}  -  OK`
                        : `${json.error?.message ?? `HTTP ${res.status}`}`));
                }
                catch (err) {
                    checks.push(new memoriesProvider_1.DoctorCheckNode(`Embedding API (${provider})`, false, String(err)));
                }
            }
            // 5. Workspace folder
            const folders = vscode.workspace.workspaceFolders;
            checks.push(new memoriesProvider_1.DoctorCheckNode('Workspace folder open', !!(folders && folders.length > 0), folders ? folders[0].uri.fsPath : 'No workspace folder open'));
        });
        ...
        if (failed === 0) {
            ...
            vscode.window.showWarningMessage(`CodeMem Doctor: ${failed} check(s) failed  -  see sidebar for details`);
        }
    }));
    // -- analyzeWorkspace ----------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.analyzeWorkspace', async () => {
        const folders = vscode.workspace.workspaceFolders;
        if (!folders || folders.length === 0) {
            vscode.window.showWarningMessage('CodeMem: open a workspace folder first');
            return;
        ...
        const workspaceRoot = folders[0].uri.fsPath;
        const namespace = (0, memoriesProvider_1.resolveNamespace)(cfg().get('namespace', '') ?? '') || folders[0].name;
        await runPureTsAnalysis(workspaceRoot, namespace, cfg, memoriesProvider, client, context);
    }));
    // -- reanalyzeChanged ---------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.reanalyzeChanged', async () => {
        const folders = vscode.workspace.workspaceFolders;
        if (!folders || folders.length === 0) {
            vscode.window.showWarningMessage('CodeMem: open a workspace folder first');
            return;
    ...
    // -- forceFullRebuild ---------------------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.forceFullRebuild', async () => {
        const folders = vscode.workspace.workspaceFolders;
        if (!folders || folders.length === 0) {
            vscode.window.showWarningMessage('CodeMem: open a workspace folder first');
            return;
    ...
    // -- File Watcher (auto incremental) ------------------------------------
    {
        ...
        if (wsFolders && wsFolders.length > 0) {
            const ignorePatterns = cfg().get('ignorePatterns', ['node_modules', '.git', 'dist', 'out', 'build', 'target']);
            const watcher = new watcher_1.AnalysisWatcher(wsFolders[0].uri.fsPath, ignorePatterns);
            watcher.start();
            context.subscriptions.push(watcher);
            watcher.onFilesChanged(async (changedFiles) => {
                const autoAnalyze = cfg().get('autoAnalyzeOnSave', false);
                if (!autoAnalyze) {
                    return;
                }
                const wsRoot = wsFolders[0].uri.fsPath;
                const ns = (0, memoriesProvider_1.resolveNamespace)(cfg().get('namespace', '') ?? '') || wsFolders[0].name;
                await runPureTsAnalysis(wsRoot, ns, cfg, memoriesProvider, client, context, { changedOnly: true, showReport: false });
            });
        }
    }
    // -- Copilot on-save hook (lightweight single-file re-index) ─────────────
    // Equivalent of Claude Code's PostToolUse/Edit hook — fires on every file
    // save and pushes updated edges to codemem without a full workspace scan.
    {
        const SUPPORTED_EXTS = new Set(['ts', 'tsx', 'js', 'jsx', 'mjs', 'cjs', 'py', 'pyw', 'rs', 'go', 'java', 'kt', 'cs']);
        const IGNORED_DIRS = ['node_modules', '.git', 'dist', 'out', 'build', 'target', '.next', '.nuxt'];
        context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(async (doc) => {
            // Only run if copilot hook indexing is enabled (default: true)
            if (!cfg().get('copilotHookIndexing', true)) {
                return;
            }
            const wsFolders = vscode.workspace.workspaceFolders;
            if (!wsFolders || wsFolders.length === 0) {
            ...
            const wsRoot = wsFolders[0].uri.fsPath;
            // Only index files inside the workspace
            if (!doc.uri.fsPath.startsWith(wsRoot)) {
                return;
            }
            // Check extension
            ...
            // Check ignored dirs
            const relPath = path.relative(wsRoot, doc.uri.fsPath).replace(/\\/g, '/');
            if (IGNORED_DIRS.some((d) => relPath.startsWith(d + '/'))) {
                return;
            }
            // Check server is reachable
            const serverUrl = cfg().get('serverUrl', 'http://localhost:4242');
            const namespace = (0, memoriesProvider_1.resolveNamespace)(cfg().get('namespace', '') ?? '') || wsFolders[0].name;
            try {
                const { parseFile: _parseFile, UploadQueue: _UploadQueue } = await Promise.resolve().then(() => __importStar(require('../analysis')));
                const content = doc.getText();
                const result = _parseFile(relPath, content, namespace);
                if (result.edges.length === 0) {
                    return;
                }
                const queue = new _UploadQueue({
                    chunkSize: cfg().get('chunkSize', 60),
                    maxRetries: 2,
                    retryDelayMs: 500,
                    namespace,
                    serverUrl,
                });
                queue.enqueue(result.edges);
                await queue.processAll();
            }
            catch {
                // Silent failure — on-save hook should never surface errors to user
            }
        }));
    }
    // -- showMemoryDetail (internal) ----------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.showMemoryDetail', (memory) => {
        showMemoryDetail(memory);
    }));
    // -- init (mirrors `codemem init`) --------------------------------------
    context.subscriptions.push(vscode.commands.registerCommand('codemem.init', async () => {
        const folders = vscode.workspace.workspaceFolders;
        if (!folders || folders.length === 0) {
            vscode.window.showWarningMessage('CodeMem: open a workspace folder first');
            return;
        ...
        const hasExe = await new Promise((resolve) => {
        ...
        for (const def of hookDefs) {
            ...
            const eventArr = hooks[def.event];
            // Check if new-style codemem mcp hook already exists
            const hasNew = eventArr.some((h) => {
                const obj = h;
                const innerHooks = obj.hooks;
                return innerHooks?.some((entry) => typeof entry.command === 'string' && entry.command.startsWith('codemem mcp '));
            });
            if (hasNew) {
                hooksSkipped++;
            }
                ...
                hooks[def.event] = eventArr.filter((h) => {
                    const innerHooks = h.hooks;
                    return !innerHooks?.some((entry) => typeof entry.command === 'string' &&
                        entry.command.startsWith('codemem ') &&
                        !entry.command.startsWith('codemem mcp '));
                });
                const hookEntry = {
                ...
                hooks[def.event].push(hookEntry);
                hooksAdded++;
        ...
        vscode.window.showInformationMessage(`CodeMem: Workspace initialized (${hooksAdded} hooks, ${agentsInstalled} agents, MCP configured)`);
    }));
}
// -- Shared helpers --------------------------------------------------------
/** Fast non-crypto hash (djb2) for edge fingerprinting */
...
async function runPureTsAnalysis(workspaceRoot, namespace, cfg, memoriesProvider, client, context, options = {}) {
    const { parseFile, analyzeComplexity, detectSmells, mapTestFiles, extractApiSurface, extractCoChangedEdges, extractModifiedByEdges, UploadQueue, showQualityReport, generateAutoMemories, } = await Promise.resolve().then(() => __importStar(require('../analysis')));
    const output = vscode.window.createOutputChannel('CodeMem Analysis', { log: true });
    const chunkSize = cfg().get('chunkSize', 60);
    const settingsIgnore = cfg().get('ignorePatterns', ['node_modules', '.git', 'dist', 'out', 'build', 'target']);
    const serverUrl = cfg().get('serverUrl', 'http://localhost:4242');
    ...
    function buildIgnoreMatcher(patterns) {
        ...
        return (relPath) => {
            ...
            if (segments.some(s => segmentNames.has(s))) {
                return true;
            }
    ...
    const prevCache = options.forceFullRebuild
        ? { files: {}, version: CACHE_VERSION }
        : (context.workspaceState.get(cacheKey) ?? { files: {}, version: CACHE_VERSION });
    function stamp(file) {
        try {
    ...
    // -- Embedding config --------------------------------------------------
    let embedUrl = cfg().get('embeddingUrl', '');
    let embedModel = cfg().get('embeddingModel', '');
    let embedKey = cfg().get('embeddingApiKey', '');
    const provider = cfg().get('embeddingProvider', '');
    ...
    if (!embedUrl || !embedModel) {
        ...
        const providerPick = await vscode.window.showQuickPick(Object.entries(PRESETS).map(([key, p]) => ({ label: p.label, description: p.url, key })), { placeHolder: 'Select embedding provider (or skip for graph-only)', title: 'CodeMem: Embedding Provider' });
        if (!providerPick) {
            return;
        }
        if (providerPick.key === 'skip') {
            ...
            if (preset.needsKey && !embedKey) {
                ...
                if (!embedKey) {
                    vscode.window.showErrorMessage('CodeMem: API key required.');
                    return;
                }
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'CodeMem: Analyzing Workspace', cancellable: true }, async (progress, token) => {
        ...
        const nextCache = { files: {}, version: CACHE_VERSION };
        const changedFiles = [];
        const allRelPaths = [];
        ...
        // If nothing changed and not forced, short-circuit
        if (changedFiles.length === 0 && !options.forceFullRebuild && !options.changedOnly) {
            output.info('  No files changed since last analysis - skipping');
            await context.workspaceState.update(cacheKey, nextCache);
            vscode.window.showInformationMessage('CodeMem: Indexing up to date - no changes detected');
            return;
        }
        ...
        if (!token.isCancellationRequested && dedupedEdges.length > 0) {
            // Compute a simple hash of all edges to detect if anything actually changed
            const edgeFingerprint = dedupedEdges.map(e => `${e.relationship}|${e.from}|${e.to}`).sort().join('\n');
            const edgeHash = simpleHash(edgeFingerprint);
            nextCache.edgeHash = edgeHash;
            if (edgeHash === prevCache.edgeHash && !options.forceFullRebuild) {
                output.info(`  Edges unchanged (hash: ${edgeHash.slice(0, 8)}...) - skipping upload`);
            }
            else {
        ...
        if (!token.isCancellationRequested && doEmbeddings) {
            ...
            for (let i = 0; i < chunks.length; i += BATCH) {
                ...
                // Store as memories
                const storeResults = await Promise.allSettled(batch.map(chunk => client.storeMemory({
        ...
        for (const e of dedupedEdges) {
            edgesByType[e.relationship] = (edgesByType[e.relationship] ?? 0) + 1;
        }
        const nodesByKind = {};
...
async function storeContent(client, content, namespace, memoriesProvider) {
    const memType = await vscode.window.showQuickPick(MEMORY_TYPES.map((t) => ({ label: t, description: typeDescription(t) })), { placeHolder: 'Memory type' });
    if (!memType) {
        return;
    }
...
async function showMemoryDetail(memory) {
    const panel = vscode.window.createWebviewPanel('codemem.memory', `Memory: ${truncate(memory.content, 30)}`, vscode.ViewColumn.Beside, {});
    const md = [
...
**Always do these at the start of each session:**
1. Call \`recall\` with the current task as the query to surface relevant prior decisions and patterns
2. Call \`session_context\` to load recent session history for this namespace

**Always do these before implementing:**
- \`search_code\` (mode: "hybrid") before writing new functions — check if a similar implementation already exists
- \`get_symbol_graph\` (direction: "incoming") before changing a function or type — understand the blast radius

**Always do these when making decisions:**
- Store architectural decisions: \`store_memory\` with \`memory_type: "decision"\`, \`importance >= 0.7\`
- Store discovered patterns: \`store_memory\` with \`memory_type: "pattern"\`
- Always pass \`links: ["sym:FunctionName", "file:path/to/file"]\` for better retrieval

**Always do these at the end of each session:**
- Call \`session_checkpoint\` to snapshot progress
